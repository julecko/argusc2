<script lang="ts">
	...
	let rows: Row[] = [
		{ name: 'payload_v2.exe',       type: 'RAT',       os: 'Windows', version: 'v2.0', size: '1.95 MB',  downloads: '5 / 10', uploadedAt: 'Mar 21, 2026, 12:02 PM' },
		{ name: 'keylogger.dll',        type: 'Keylogger', os: 'Windows', version: 'v1.0', size: '500.00 KB', downloads: '3 / 5',  uploadedAt: 'Mar 20, 2026, 12:02 PM' },
		{ name: 'screenshot_tool.py',   type: 'Spyware',   os: 'Linux',   version: 'v1.0', size: '15.00 KB',  downloads: '2 / ∞',  uploadedAt: 'Mar 19, 2026, 12:02 PM' },
	];
	...
	const osColors: Record<string, BadgeVariant> = {
		Windows: 'gray',
		Linux:   'gray',
		macOS:   'gray',
	};
	...
	<DataTable {columns} {rows} statusKey="__none__">
		<svelte:fragment slot="cell" let:col let:row let:value>
			...
			{:else if col.key === 'type'}
				<span class="dt-badge dt-badge--{typeColors[String(value)] ?? 'gray'}">
					{value}
				</span>

			{:else if col.key === 'os'}
				<span class="dt-badge dt-badge--gray os-badge">
					{value}
				</span>

			{:else if col.key === 'version'}
				<span class="dt-badge dt-badge--gray version-badge">
					{value}
				</span>
...
<style lang="scss">
	...
	.dt-badge {
		...
		&--red    { background: #2b0a0a; color: #f87171; border-color: #7a1f1f; }
		&--amber  { background: #2b1f04; color: #f5a623; border-color: #7a4f08; }
		&--gray   { background: #1e1e26; color: #6b6b7e; border-color: #2e2e3e; }
		&--blue   { background: #0a1628; color: #60a5fa; border-color: #1e3a6e; }
		&--green  { background: #0d2b1a; color: #34d87a; border-color: #1a5c35; }
	}
